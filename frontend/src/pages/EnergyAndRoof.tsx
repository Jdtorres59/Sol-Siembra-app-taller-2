...
import { getEnergyData, sendRoofCommand } from '../services/api';

const EnergyAndRoof = () => {
  const [roofData, setRoofData] = useState(roofStatus);
  const [energyData, setEnergyData] = useState(energyStatus);

  useEffect(() => {
    getEnergyData().then((payload) => {
      setEnergyData(payload.energyStatus);
      setRoofData(payload.roofStatus);
    });
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card title="Generación solar actual" subtitle="kW disponibles">
          <p className="text-3xl font-semibold text-gray-900">{energyData.currentGenerationKw} kW</p>
          <p className="text-sm text-gray-500">Promedio de la última hora</p>
        </Card>
        <Card title="Energía generada hoy" subtitle="kWh acumulados">
          <p className="text-3xl font-semibold text-gray-900">{energyData.generatedTodayKwh} kWh</p>
        ...
        <Card title="Estado de baterías" subtitle="Autonomía estimada">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-3xl font-semibold text-gray-900">{energyData.batteriesPercent}%</p>
              <p className="text-sm text-gray-500">{energyData.autonomyHours} h de respaldo</p>
            </div>
            <div className="h-16 w-3 rounded-full bg-gray-100">
              <div
                className="h-full w-full rounded-full bg-brand-primary"
                style={{ height: `${energyData.batteriesPercent}%` }}
              />
            </div>
          </div>
        </Card>
      ...
      <Card title="Curva solar del día" subtitle="Potencia generada por hora">
        <div className="h-72">
          <ResponsiveContainer>
            <AreaChart data={energyData.solarCurve}>
              <defs>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {energyData.consumptionTable.map((row) => (
                  <tr key={row.day}>
