...
const HistoryAndReports = () => {
  ...
  const filteredEvents = useMemo(() => {
    if (filters.zoneId === 'all') return historyData.eventLogs;
    const zone = irrigationZones.find((z) => z.id === filters.zoneId);
    if (!zone) return historyData.eventLogs;
    return historyData.eventLogs.filter((event) => event.zone === zone.name);
  }, [filters.zoneId, historyData.eventLogs]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card title="Consumo de agua" subtitle="m³ aplicados por día">
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={historyData.historySeries}>
        ...
        <Card title="Rendimiento hídrico" subtitle="kg de arroz por m³ de agua">
          <div className="h-64">
            <ResponsiveContainer>
              <LineChart data={historyData.historySeries}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                <XAxis dataKey="day" stroke="#9CA3AF" />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card title="Ahorro de agua" subtitle="vs riego tradicional">
          <p className="text-4xl font-semibold text-brand-primary">{historyData.historyKpis.waterSaving}%</p>
        ...
        <Card title="Reducción de pérdidas" subtitle="Eventos climáticos evitados">
          <p className="text-4xl font-semibold text-brand-primary">{historyData.historyKpis.lossReduction}%</p>
          <p className="text-sm text-gray-500">Estimación por sensores</p>
        </Card>
        <Card title="Operatividad del sistema" subtitle="Disponibilidad">
          <p className="text-4xl font-semibold text-brand-primary">{historyData.historyKpis.uptime}%</p>
