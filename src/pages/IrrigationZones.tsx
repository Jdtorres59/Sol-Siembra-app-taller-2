...
const IrrigationZones = () => {
  ...
  const selectedZone = useMemo(
    () => irrigationZones.find((zone) => zone.id === selectedZoneId) ?? irrigationZones[0],
    [selectedZoneId],
  );

  const selectedHistory = useMemo(
    () => zoneHistory.find((zone) => zone.zoneId === selectedZone?.id)?.readings ?? [],
    [selectedZone?.id],
  );

  const zoneConfig = irrigationConfigByZone[selectedZone?.id ?? ''] ?? {
    minMoisture: 0,
    windows: [],
